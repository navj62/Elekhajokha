...
- **`--muted-foreground-subtle` fails contrast on every dark surface.** The third text level is `#908D81` on dark, which measures **3.33:1 on `--card`, 3.18:1 on `--card-alt`, 2.99:1 on `--background`** — all below the 4.5:1 needed for normal-size text, and it is used for small uppercase labels. This is a **token-level** failure, so unlike the hardcoded-hex issues in DARK_MODE_AUDIT.md it is not fixed by migrating a screen: a fully-migrated file using the correct token still fails. Fixing it means lightening the dark value, not editing screens. (Light mode is fine: 5.02:1 on card, 4.81:1 on card-alt, 4.52:1 on background.)
