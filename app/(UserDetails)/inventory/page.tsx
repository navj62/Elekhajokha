...
function DirectPurchaseModal({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!form.itemType) { setError("Item type is required."); return; }
    if (isNaN(wg) || wg <= 0) { setError("Weight must be > 0."); return; }
    if (isNaN(ac) || ac < 0) { setError("Purchase price must be ≥ 0."); return; }
    ...
    try {
      ...
      if (form.sellerName.trim()) fd.append("sellerName",  form.sellerName.trim());
  ...
  const allTypes = [...itemTypes.defaults, ...itemTypes.custom];

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div
        className="relative w-full max-w-[600px] max-h-[90vh] overflow-y-auto rounded-[18px] p-7"
      ...
      >
        ...
        <p className="text-[12.5px] mb-6" style={{ color: "var(--text-secondary)" }}>
          Record an item purchased directly (not from a pledge).
        </p>
            ...
            <div>
              ...
              <div className="relative">
                <select
                  ...
                  className="w-full px-3 py-2 rounded-[10px] text-[13px] outline-none appearance-none focus:ring-2 focus:ring-[#A2AB89]"
                  style={{
                    backgroundColor: "var(--main-bg)",
                    border: "1px solid var(--border-light)",
                    color: "var(--text-primary)",
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                onChange={(e) => setForm((f) => ({ ...f, acquiredCost: e.target.value }))}
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setForm((f) => ({ ...f, sellerIdNum: e.target.value }))}
