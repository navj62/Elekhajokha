...
export async function PATCH(req: Request, context: RouteContext) {
  try {
    ...
    // "numeric field overflow" and rolls back the whole release transaction.
    // A value AT the ceiling means "at or above 999.99%", never a precise figure.
    const ltvAtRelease =
...
export async function GET(_req: Request, context: RouteContext) {
  try {
    ...
    // This pledge's part-payment history (ownership enforced via relation).
